...
description: An iOS-style toggle switch for theme switching with animated transitions.
---

An iOS-style toggle switch with a sliding thumb. Supports all built-in transitions via the `transition` prop, or pass custom CSS via the `css` prop.
...
```shell frame="none"
npx shadcn@latest add @transitions/theme-toggle-switch
```
...
```shell frame="none"
yarn dlx shadcn@latest add @transitions/theme-toggle-switch
```
...
```shell frame="none"
pnpm dlx shadcn@latest add @transitions/theme-toggle-switch
```
...
```shell frame="none"
bunx shadcn@latest add @transitions/theme-toggle-switch
```
...
```tsx title="src/components/ThemeToggleSwitch.tsx"
"use client";
...
{/* @build-registry:start */}
```vue title="src/components/ThemeToggleSwitch.vue"
...
{/* @build-registry:start */}
```svelte title="src/components/ThemeToggleSwitch.svelte"
<script lang="ts">
...
{/* @build-registry:start */}
```ts title="src/components/ThemeToggleSwitch.ts"
function getCurrentTheme(): "light" | "dark" {
	const el = document.documentElement;
	if (el.classList.contains("dark")) return "dark";
	if (el.classList.contains("light")) return "light";
...
export default function App() {
  return (
    <div>
      <ThemeToggleSwitch transition="fade" />
      <ThemeToggleSwitch transition="circle-reveal" />
      <ThemeToggleSwitch transition="scale" duration={500} />
    </div>
  );
}
```
