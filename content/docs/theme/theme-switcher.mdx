...
description: A segmented control with System/Light/Dark options and animated theme transitions.
---

A three-option switcher (System, Light, Dark) with a sliding indicator. Supports all built-in transitions via the `transition` prop, or pass custom CSS via the `css` prop.
...
```shell frame="none"
npx shadcn@latest add @transitions/theme-switcher
```
...
```shell frame="none"
yarn dlx shadcn@latest add @transitions/theme-switcher
```
...
```shell frame="none"
pnpm dlx shadcn@latest add @transitions/theme-switcher
```
...
```shell frame="none"
bunx shadcn@latest add @transitions/theme-switcher
```
...
```tsx title="src/components/ThemeSwitcher.tsx"
"use client";
...
{/* @build-registry:start */}
```vue title="src/components/ThemeSwitcher.vue"
...
{/* @build-registry:start */}
```svelte title="src/components/ThemeSwitcher.svelte"
<script lang="ts">
...
{/* @build-registry:start */}
```ts title="src/components/ThemeSwitcher.ts"
function getCurrentTheme(): "light" | "dark" {
	const el = document.documentElement;
	if (el.classList.contains("dark")) return "dark";
	if (el.classList.contains("light")) return "light";
...
See the [full transitions table](/components) for all available slugs, categories, and descriptions.
...
export default function App() {
  return (
    <div>
      <ThemeSwitcher transition="fade" />
      <ThemeSwitcher transition="circle-reveal" />
      <ThemeSwitcher transition="scale" duration={500} />
    </div>
  );
}
```
...
- Stores the preference in `localStorage` and cookies for SSR compatibility
- `system` resolves to the OS `prefers-color-scheme` at selection time

### Accessibility

- Uses `aria-label` and `aria-pressed` on each option
