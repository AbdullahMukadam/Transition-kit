...
description: A theme toggler button with animated transitions using the View Transitions API.
---

A button that toggles between light and dark modes with a Sun/Moon icon animation. Supports all built-in transitions via the `transition` prop, or pass custom CSS via the `css` prop.
...
```shell frame="none"
npx shadcn@latest add @transitions/animated-theme-toggler
```
...
```shell frame="none"
yarn dlx shadcn@latest add @transitions/animated-theme-toggler
```
...
```shell frame="none"
pnpm dlx shadcn@latest add @transitions/animated-theme-toggler
```
...
```shell frame="none"
bunx shadcn@latest add @transitions/animated-theme-toggler
```
...
```tsx title="src/components/AnimatedThemeToggler.tsx"
"use client";
...
{/* @build-registry:start */}
```vue title="src/components/AnimatedThemeToggler.vue"
...
{/* @build-registry:start */}
```svelte title="src/components/AnimatedThemeToggler.svelte"
<script lang="ts">
...
{/* @build-registry:start */}
```ts title="src/components/AnimatedThemeToggler.ts"
function getCurrentTheme(): "light" | "dark" {
	const el = document.documentElement;
	if (el.classList.contains("dark")) return "dark";
	if (el.classList.contains("light")) return "light";
...
export default function App() {
  return (
    <div>
      <AnimatedThemeToggler transition="fade" />
      <AnimatedThemeToggler transition="circle-reveal" />
      <AnimatedThemeToggler transition="scale" duration={500} />
    </div>
  );
}
```
