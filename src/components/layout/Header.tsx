...
export default function Header() {
	...
	return (
		<div className="flex justify-center w-full sticky top-0 z-50 rise-in">
			<motion.header
				animate={{
					width: isDesktop ? (scrolled ? "45%" : "55%") : "95%",
					y: scrolled ? 10 : 0,
					borderRadius: scrolled ? "2.5rem" : "0.5rem",
				}}
				transition={{
					duration: 0.3,
					ease: "easeOut",
				...
				className={cn(
					"relative backdrop-blur-2xl bg-[var(--background)]/80 border",
					"text-[var(--foreground)] transition-[border-color,box-shadow,background-color] duration-300",
					scrolled
						? "border-[var(--border)]/40 shadow-md"
						: "border-transparent",
				)}
			>
				<div
					aria-hidden
					className={cn(
						"pointer-events-none absolute inset-x-2 bottom-0 h-px transition-opacity duration-300",
						"bg-[repeating-linear-gradient(135deg,var(--border)_0px_1px,transparent_1px_10px)]",
