...
function CodeSection({ label, code, variant }: { label: string; code: string | ((variant: string) => string); variant?: string }) {
    ...
    return (
        <div className="my-6 w-full rounded-lg border bg-background shadow-sm">
            <Tabs defaultValue="preview" className="w-full flex flex-col">
                ...
                {/* Code Tab */}
                <TabsContent value="code" className="m-0 border-t bg-muted/20 p-6 space-y-6 rounded-b-lg">
                    {installCode && <CodeSection label="Installation" code={installCode} />}
                    {usageCode && <CodeSection label="Usage" code={usageCode} variant={selectedVariant} />}
                </TabsContent>
            </Tabs>
        </div>
    );
